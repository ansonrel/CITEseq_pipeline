---
title: "Look at Expression markers in azimuth-defined celltypes"
...
#load object after running azimuth online
annots <- read.table(sprintf("sce/%s/azimuth_pred.tsv", linage), sep = '\t', header = TRUE)

#load RNA-filtered SCE
#d <- readRDS(sprintf("sce/%s/d_rna_filt.rds", linage))
d <- readRDS(sprintf("sce/%s/d_adt_clust.rds", linage))
#d <- d[, Cells(umap)]
#reducedDim(d, "UMAP_azimuth") <- umap@cell.embeddings
...
saveRDS(d, sprintf("sce/%s/d_adt_annot.rds", linage))
```

## Look at RNA UMAP with azimuth annotations
...
dir.create(sprintf("reports/%s/TABLES/", linage))
...
samecols <- sapply(celltypes, function(u){
    ...
    if(nrow(subdf) > 0) {
    write.csv(subdf, 
              file = sprintf("reports/%s/TABLES/DE_genes_against_%s_%s.csv",
                             linage,testf, u))
    }
  })
...
metadata(d)$cluster_codes <- data.frame(
  ...
  azimuth = factor(unique(d$predicted.id)))

idx <- match(d$predicted.id, metadata(d)$cluster_codes$azimuth)
...
#cluster heatmap
plotExprHeatmap(dsub, by = "cluster_id", k = "custom", m = "azimuth",
...
dir.create(sprintf("reports/%s/HEATMAPS",linage))
#png("test.png", width = 1000, height = 1500)
pdf(sprintf("reports/%s/HEATMAPS/heatmap_markers_long.pdf",linage), width = 11, height = 14)
